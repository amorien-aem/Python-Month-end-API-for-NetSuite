...
if os.environ.get('HEADLESS'):
    tasks = [
        ...
        "Review and clear any uncleared transactions",
        "Post Bank Service Charges, Interest, etc.",
    ]
    import json, os
    os.makedirs('output', exist_ok=True)
    out = {'script':'4BankReconciliation.py','summary':f'{len(tasks)} checklist items','items':tasks}
    with open(os.path.join('output','4BankReconciliation.json'),'w') as f:
        json.dump(out, f)
    import csv
    with open(os.path.join('output','4BankReconciliation.csv'),'w', newline='') as cf:
        w = csv.writer(cf)
        w.writerow(['item','completed'])
        for t in tasks:
            w.writerow([t,''])
    print(f"4BankReconciliation.py: HEADLESS summary written to output/4BankReconciliation.json and CSV")
    sys.exit(0)
...
# Title
title_label = ttk.Label(frame, text="4 Bank Reconciliation", font=("Arial", 16))
